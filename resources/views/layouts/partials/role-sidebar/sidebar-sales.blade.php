@can('menu-manager')
    <li class="menu-title" data-key="t-menu">ROLE MENU : MANAGER</li>
@endcan

@can('dashboard-manager-sales')
    <li>
        <a href="{{ route('sales-performace-report-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Dashboard</span>
        </a>
    </li>
@endcan
@can('customer-management')
    <li>
        <a href="{{ route('leads-customer-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Customer Management</span>
        </a>
    </li>
@endcan
@can('quotation-management')
    <li>
        <a href="{{ route('quotation-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Quotation Management</span>
        </a>
    </li>
@endcan
@can('order-management')
    <li>
        <a href="{{ route('order-management-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Order Management</span>
        </a>
    </li>
@endcan
@can('sales-team-management')
    <li>
        <a href="#">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Sales Team Management</span>
        </a>
    </li>
@endcan
@can('discount-&-Promotion-Management')
    <li>
        <a href="#">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Discount & Promotion Management</span>
        </a>
    </li>
@endcan

@can('list-office-inventory')
    <li>
        <a href="{{ route('office-inventory-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Office Inventory</span>
        </a>
    </li>
@endcan


@can('menu-staff')
    <li class="menu-title" data-key="t-menu">ROLE MENU : STAFF</li>
@endcan
@can('dashboard-staff-sales')
    <li>
        <a href="{{ route('sales-performace-report-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Dashboard</span>
        </a>
    </li>
@endcan
@can('customer-management')
    <li>
        <a href="{{ route('leads-customer-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Customer Management</span>
        </a>
    </li>
@endcan
@can('quotation-management')
    <li>
        <a href="{{ route('quotation-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Quotation Management</span>
        </a>
    </li>
@endcan
@can('order-management')
    <li>
        <a href="{{ route('order-management-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Order Management</span>
        </a>
    </li>
@endcan
@can('sales-task')
    <li>
        <a href="{{ route('sales-task-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Sales Task</span>
        </a>
    </li>
@endcan


@can('menu-analytics-manager')
    <li class="menu-title" data-key="t-menu">ANALYTICS MENU : MANAGER</li>
@endcan

@can('sales-performance-reports')
    <li>
        <a href="{{ route('sales-performace-report-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Sales Performance Reports</span>
        </a>
    </li>
@endcan
@can('kpi-tracking')
    <li>
        <a href="#">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">KPI Tracking</span>
        </a>
    </li>
@endcan
@can('customer-analytics')
    <li>
        <a href="#">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Customer Analytics</span>
        </a>
    </li>
@endcan
@can('quotation-&-order-analytics')
    <li>
        <a href="{{ route('sales-performace-report-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Quotation & Order Analytics</span>
        </a>
    </li>
@endcan

@can('menu-analytics-staff')
    <li class="menu-title" data-key="t-menu">ANALYTICS MENU : STAFF</li>
@endcan
@can('personal-sales-reports')
    <li>
        <a href="{{ route('sales-performace-report-list') }}">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Personal Sales Reports</span>
        </a>
    </li>
@endcan
@can('customer-interaction-analysis')
    <li>
        <a href="#">
            <i class="uil-users-alt nav-icon"></i>
            <span class="menu-item" data-key="t-authentication">Customer Interaction Analysis</span>
        </a>
    </li>
@endcan


